 ...
 * 시도 별 선택 이벤트
 */
$(document).on("click", ".district-select-insert", function() {
	
	$(this).removeClass("btn-outline-blue");
	$(this).addClass("btn-blue");
	
	$(this).siblings("label").removeClass("btn-blue");
	$(this).siblings("label").addClass("btn-outline-blue");
	
	$(".district-view").removeClass("focus");
	$("#district-view-"+$("input:radio[name=districtSelect]:checked").val() ).addClass("focus");
	console.log("#district-view-"+$("input:radio[name=districtSelect]:checked").val() );
	
	fn_resizeOffcanvasTable();
});

$(document).on("click", ".district-select-update", function() {
	...
	$(".district-view").removeClass("focus");
	$(".district-view-"+$("input:radio[name=districtSelect_u]:checked").val() ).addClass("focus");
	console.log(".district-view-"+$("input:radio[name=districtSelect_u]:checked").val() );
	...
	if(indiName.length > 0) {
		...
		if(indiName.length < 30) {
			...
			if(regText.test(indiName)) {
				...
				$.ajax({
					url: "/admin/system/indicator/isIndiName.do",
...
function fn_IndicatorList(page) {
	...
	$.ajax({
		url: "/admin/system/indicator/fieldIndicator.do",
		type: "get",
		data: {page: page},
		dataType: "json",
		success: function(data) {
...
function fn_indicatorCategory(id) {
	...
	$.ajax({
		url: "/admin/system/indicator/indicatorUpdateData.do",
		type: "get",
		...
		success: function(data) {
			...
			var indicatorListInfo = data.indicatorListInfo;
			console.log(indicatorDataList);
			var collision = true;
			var before = 0 ;
			/** 지표값 */
			for(var i = 0; i < indicatorDataList.length; i++) {
				console.log(before + " / " + collision);
				if(before == indicatorDataList[i].district_cd.substring(0,2)){
					collision = false;
				}else{
					collision = true;
				}
				
				if(indicatorDataList[i].district_cd.substring(0,2) == '11' && collision){
					var $content11 = $('<div class="col-12 mb-3 p-3 district-view district-view-11 focus"></div>');
					...
					$content11.append($row);
					before = indicatorDataList[i].district_cd.substring(0,2);
					$("#update-indicator-value").append($content11);	
					collision = false;
					
				}else if(indicatorDataList[i].district_cd.substring(0,2) != '11' && collision){
					var $content = $('<div class="col-12 mb-3 p-3 district-view district-view-'+ indicatorDataList[i].district_cd.substring(0,2) +'"></div>');
					var $row = $('<div class="row mb-3"></div>');
					var $container = $('<div class="col-6 mb-3"></div>');
					var $div = $('<div class="mb-1">' + indicatorDataList[i].district_nm + '</div>');
					var $input = $('<input type="text" class="form-control indicator-value"'
					...
					$row.append($container);
					$content.append($row);
					$("#update-indicator-value").append($content);
					...
					$row.append($container);
					before = indicatorDataList[i].district_cd.substring(0,2);
					$("#update-indicator-value > div.district-view-"+indicatorDataList[i].district_cd.substring(0,2)+" > div").append($container);
				}
				
				
				//$("#update-indicator-value > .district-view-11 > div").append($container);
			}
	...
	if(len > 0) {
		...
		if((navigator.appName == "Netscape" && navigator.userAgent.search("Trident") != -1) || (agent.indexOf("msie") != -1)) {
			...
			if(confirm("[확인] 정말 지표를 삭제 하겠습니까?")) {
				...
				location.href="/admin/system/indicator/delete.do?indi_id=" + indi_id;
			}
