...
function fn_groupIndicator(group_id, page) {
	...
	$.ajax({
		url: "/member/base/custom/item/indicator.do",
...
function fn_isItemName() {
	...
	if($("#custom-form").attr("action").indexOf("/custom/item/update.do") > -1) {
	...
	if(itemName.length > 0) {
		...
		if(itemName.length < 30) {
			...
			if(regText.test(itemName)) {
				...
				$.ajax({
					url: "/member/base/custom/item/isItemName.do",
					data: params,
					type: "get",
					dataType: "json",
					success: function(data) {
...
function fn_itemInfo(item_id) {
	...
	$.ajax({
		url: "/member/base/custom/item/itemInfo.do",
		type: "get",
...
function fn_itemCategory(id) {
	...
	if(id == "item-update-view") {
		...
		if(len < 1) {
			...
			$("#custom-form").attr("action", "/member/base/custom/item/update.do?${_csrf.parameterName}=${_csrf.token}");
		...
		offcanvasClose();
		$("#custom-form").attr("action", "/member/base/custom/item/insert.do?${_csrf.parameterName}=${_csrf.token}");
	}
}
	...
	if(len > 0) {
		...
		if((navigator.appName == "Netscape" && navigator.userAgent.search("Trident") != -1) || (agent.indexOf("msie") != -1)) {
			...
			if(confirm("[확인] 정말 항목을 삭제 하겠습니까?")) {
				location.href="/member/base/custom/item/delete.do?item_id=" + item_id;
			}
...
$(document).on("change", "#copy-template-field", function() {
	...
	$.ajax({
		url: "/member/base/custom/item/templateItem.do",
		type: "get",
