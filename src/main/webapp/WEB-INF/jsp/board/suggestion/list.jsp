		...
		<td class="col align-top p-0">
			...
			<div class="mainContents"><!-- mainContents -->
				...
				<div class="navbar p-0 pb-3">
					...
					<div class="contentsTitle"><i class="icon-user-talk"> </i>건의사항</div>
				...
				<table class="table table-hover vestapTable mainTable">
					...
					<c:if test="${fn:length(boardList) eq 0 }">
						<tr>
							<td colspan="5">등록 된 건의사항이 없습니다.</td>
						</tr>
					</c:if>
					
					<c:forEach items="${boardList }" var="list">
						<c:if test="${list.bbs_use eq 'Y' }">
							<tr class="offcanvas-select" onclick="javascript:linkContent('/member/base/board/suggestion/content.do', '${list.bbs_idx}');">
								<td><c:out value="${list.RNUM }"/></td>
								<c:choose>
									<c:when test="${list.bbs_ref eq 0 }">
										<td class="t-overflow category-title search-nm"><c:out value="${list.bbs_title }"/></td>
									</c:when>
									<c:otherwise>
									
											<c:if test="${list.bbs_depth le 5 }">
										
												<td class="t-overflow category-title pl-${list.bbs_depth + 3 } search-nm"><c:out value="${list.bbs_title }"/></td>
											</c:if>
											<c:if test="${list.bbs_depth gt 5 }">
												<td class="t-overflow category-title pl-7 search-nm"><c:out value="${list.bbs_title }"/></td>
											</c:if>
						...
						<c:if test="${list.bbs_use eq 'N' }">
							<tr style="color: #8d949c;">
								<td><c:out value="${list.RNUM }"/></td>
									<c:choose>
										<c:when test="${list.bbs_ref eq 0 }">
											<td class="t-overflow category-title search-nm"><i class="far fa-trash-alt"></i>&ensp;<c:out value="${list.bbs_title }"/></td>
										</c:when>
										<c:otherwise>
										
												<c:if test="${list.bbs_depth le 5 }">
											
													<td class="t-overflow category-title pl-${list.bbs_depth + 3 } search-nm"><i class="far fa-trash-alt"></i>&ensp;<c:out value="${list.bbs_title }"/></td>
												</c:if>
												<c:if test="${list.bbs_depth gt 5 }">
													<td class="t-overflow category-title pl-7 search-nm"><i class="far fa-trash-alt"></i>&ensp;<c:out value="${list.bbs_title }"/></td>
												</c:if>
				...
				<sec:authorize access="hasAnyRole('ROLE_WIDE', 'ROLE_BASE')">
					<div class="nav navbar p-0 mt-4">
						...
						<button type="button" class="btn btn-vestap btn-blue w-auto pl-2 pr-2" onclick="location.href='/member/base/board/suggestion/write.do'"><i class="icon-add-bold"></i>건의사항 등록</button>
					</div>
				</sec:authorize>
			...
			<div class="card notice board-content" style="height: 1450px"><!-- 리스트 수가 20개이거나 20개나 안될 경우 높이 입니다 -->
				...
				<ul class="list-group list-group-flush">
					...
					<!-- 관리자 접속시 -->
					<sec:authorize access="hasAnyRole('ROLE_ADMIN', 'ROLE_WIDE', 'ROLE_BASE')">
					<li class="list-group-item p-4">
						
						<span class="borad-dete float-right">
							<span class="borad-dete-val bbs_view" id="bbs_management">
								<!-- 
								<button type="button" class="btn btn-vestap btn-blue w-auto pl-2 pr-2 mr-1" onclick="location.href='/member/base/board/suggestion/delete.do'"><i class="icon-folder-delete-01"></i>삭제</button>
...
<form id="listFrm" action="/member/base/board/suggestion/list.do">
