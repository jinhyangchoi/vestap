...
<table class="offcanvas-table">
	<tr>
							...
							<div class="col">
								...
								<div class="toolTipUl">
									...
									<c:forEach var="i" items="${itemList }">
										<li>
											<a href='#' class='<c:if test="${setItem ne null and setItem eq i.itemId }">active</c:if>'
											data='${i.itemId }' onclick='fn_itemClick($(this));return false;'>${i.itemNm }
											<c:if test="${i.itemId == 'TL000060' || i.itemId == 'TL000061' || i.itemId == 'TL000062' || i.itemId == 'TL000063' || i.itemId == 'TL000064' || i.itemId == 'TL000065' || i.itemId == 'TL000066' || i.itemId == 'TL000067' || i.itemId == 'TL000068' || i.itemId == 'TL000069' || i.itemId == 'TL000070' || i.itemId == 'TL000071'}">
		...
		<td class="col offcanvas-center align-top p-0">
								...
								<table class="table vestapTable smTable text-center">
									<thead>
										<tr>
											<th>번호</th>
											<th>행정구역 명칭</th>
											<th style="width:210px" id ="indiInfo">지표명</th>
...
$(document).on("click", "input:radio[name='legendChange']", function() {
	$("input:radio[name='legendChange']").attr('checked', false);
	var state = $(this).val();
...
function fn_resultViewSetting(){
	...
	if($("#itemList a.active").text().indexOf("new") > 0){
		$("._item").text($("#itemList a.active").text().replace("new",""));
	}else{
...
function fn_itemList(){
	...
	$.ajax({
		 ...
		 success:function(result){
           ...
           for(var i in list){
        	   if(list[i].itemId == 'TL000060' || list[i].itemId == 'TL000061'|| list[i].itemId == 'TL000062'|| list[i].itemId == 'TL000063'
        			   || list[i].itemId == 'TL000064'|| list[i].itemId == 'TL000065'|| list[i].itemId == 'TL000066'|| list[i].itemId == 'TL000067'
        			   || list[i].itemId == 'TL000068'|| list[i].itemId == 'TL000069'|| list[i].itemId == 'TL000070'|| list[i].itemId == 'TL000071'){
        		   $("#itemList").append("<li><a href='#' data='"+list[i].itemId+"' onclick='fn_itemClick($(this));return false;'>"+list[i].itemNm+"<sub style='color:red;'>new</sub></a></li>");
        	   }else{
...
function fn_modelList(){
	...
	$.ajax({
		 url:'/member/base/climate/estimation/modelList.do',
		 dataType :"json",
		 type : "get",
		 data : dataSet,
		 success:function(result){
...
function fn_estimationTotal(exe){
	...
	$.ajax({
		...
		success:function(result){
			...
			if(sigungu!='' && sigungu!=null){
				var radat_indiList = new Array();
				radat_indiList.push("x");
				...
				for(var i in list){
					...
					for(var j in indi_wei_list){
						...
						if(indi_wei_list[j].districtCd == list[i].districtCd) {
							//지표명 리스트 추가
							if(!radat_indiList.includes(indi_wei_list[j].indiNm)){
								radat_indiList.push(indi_wei_list[j].indiNm);	
							}
					...
					if(i==0){
						...
						//fn_radarRedraw(totalSggNm, totalClimValue, totalSensValue, totalAdaptValue);
						fn_indiChartDraw(radat_indiList,climVal, sensVal,adptVal);
					}
					...
					html+='<td>'+list[i].no+'</td><td onclick="fn_subEstimation(\''+list[i].districtCd+'\');return false;" >'+list[i].emdNm+'</td><td>'+list[i].estiValue+'</td><td>'+list[i].climValue+'</td><td>'+list[i].sensValue+'</td><td>'+list[i].adapValue+'</td><td onclick="fn_estiResultSet(\''+Object.values(radat_indiList)+'\',\''+Object.values(climVal)+'\',\''+Object.values(sensVal)+'\',\''+Object.values(adptVal)+'\',\''+ list[i].emdNm + '\','+ list[i].climValue + ',' + list[i].sensValue + ',' + list[i].adapValue +');return false;"><i class="icon-graph-pie i-24"></i></td></tr>';
				...
				for(var i in list){
					...
					html+='<td>'+list[i].no+'</td><td onclick="fn_subEstimation(\''+list[i].districtCd+'\');return false;" >'+list[i].sggNm+'</td><td>'+list[i].estiValue+'</td><td>'+list[i].climValue+'</td><td>'+list[i].sensValue+'</td><td>'+list[i].adapValue+'</td><td onclick="fn_estiResultSet(\''+Object.values(radat_indiList)+'\',\''+Object.values(climVal)+'\',\''+Object.values(sensVal)+'\',\''+Object.values(adptVal)+'\', \''+ list[i].sggNm +'\','+ list[i].climValue + ',' + list[i].sensValue + ',' + list[i].adapValue +' );return false;"><i class="icon-graph-pie i-24"></i></td></tr>';
			...
			addVector(layer_cd,layer_val,onlyCoastNear);
	...
	$.ajax({
		...
		success:function(result){
			...
			for(var i in list){
				...
				/*지표별 방사형그래프그리기 위해 데이터 배열삽입*/
				 for(var j in indi_wei_list){
						
					if(indi_wei_list[j].districtCd.substr(0,4) == list[i].districtCd) {
						//지표명 리스트 추가
				...
				if(i%pageLimit==0) page++;
				html +='<tr class="pg'+page+'"  onclick="fn_estiResultSet(\''+ Object.values(radar_indiList)+'\',\''+Object.values(climVal)+'\',\''+Object.values(sensVal)+'\',\''+Object.values(adptVal)+'\', \''+list[i].sggNm+'\','+list[i].climValue+','+list[i].sensValue+','+list[i].adapValue+');return false;"'
				if(page==1){
					html+=' style="display:table-row">';
				}else{
					html+=' style="display:none">';
...
function fn_barDraw(indiList, value){
...
function fn_indiData(){
	...
	$.ajax({
		...
		success:function(result){
			...
			var cnt = 0;
			for(var i in list){
				var indiVal = list[i].indiVal;
				if(indiVal==null)cnt++;
			}
			if(cnt>list.length/2){
				$("#resultIndiInfo").append('<tr><td class="text-center" colspan ="4">* 취약성평가에 적용되지않는 모델 데이터입니다.</td></tr>');
			}else{
			...
			if(type=='EMD'){
				for(var i in list){
					var indiVal = list[i].indiVal;
					var indiYear = list[i].indiYear;
					if(indiVal==null)indiVal='데이터 없음';
					if(indiYear==null)indiYear='데이터 없음';
					if(i%pageLimit==0) page++;
					if(page==1){
						$("#resultIndiInfo").append('<tr class="pg'+page+'" style="display:table-row"><td>'+list[i].no+'</td><td>'+list[i].emdNm+'</td><td>'+indiVal+'</td><td>'+indiYear+'</td></tr>');
					}else{
						$("#resultIndiInfo").append('<tr class="pg'+page+'" style="display:none"><td>'+list[i].no+'</td><td>'+list[i].emdNm+'</td><td>'+indiVal+'</td><td>'+indiYear+'</td></tr>');
					}
				}
			}else{
				for(var i in list){
					...
					if(i%pageLimit==0) page++;
					if(page==1){
						$("#resultIndiInfo").append('<tr class="pg'+page+'" style="display:table-row"><td>'+list[i].no+'</td><td>'+list[i].sggNm+'</td><td>'+indiVal+'</td><td>'+indiYear+'</td></tr>');
					}else{
						$("#resultIndiInfo").append('<tr class="pg'+page+'" style="display:none"><td>'+list[i].no+'</td><td>'+list[i].sggNm+'</td><td>'+indiVal+'</td><td>'+indiYear+'</td></tr>');
					}
				}
			}
...
function fn_printReport(){
	...
	if((navigator.appName == "Netscape" && navigator.userAgent.search("Trident") != -1) || (agent.indexOf("msie") != -1)) {
		...
		if(confirm("보고서 내 지도는 현재 화면에 보여지는 지도를 기준으로 제공됩니다.")) {
			fn_report();
		}
		...
		swal({
			text: "보고서 내 지도는 현재 화면에 보여지는 지도를 기준으로 제공됩니다.",
			type: 'info',
...
function fn_report(){
    ...
    form.setAttribute("class","report-form");
	...
	html2canvas(document.getElementById("esti_map")).then(function(canvas){
		...
		img = imgData.toString();
		
		$(".report-form").prepend($('<input type="hidden" name="map" value="' + img + '">'));
...
function fn_excelDownload(type){
	...
	var item = $("#selectItem").val();
	if($("#itemList a.active").text().indexOf("new") > 0){
		var itemNm = $("#itemList a.active").text().replace("new","");
	}else{
	...
	var chart = bb.generate({
		padding: {
		  ...
		  data: {
		    ...
		    columns: [
				["x", "1일 최대 강수량", "일강수량 \n110mm이상인\n 날의 수", "연속적인 강우 일수", "산사태위험지역\n 내 건축물수", "산림대비 산불발생현황","공항 면적", "목지 면적", "재정 자주도", "지역내 총생산", "위험지역 \n예방대책 \n평가등급","하천개수율","전기 공급설비 면적"],
		    ],
		    type: "radar"
		  },
		  radar: {
