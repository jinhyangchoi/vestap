...
function fn_estimationTotal(exe){
	...
	if(sigungu!='' && sigungu!=null){
		url = '/member/base/climate/estimation/resultEmdTotal2.do';
	}else{
		url = '/member/base/climate/estimation/resultSggTotal2.do';
	}
	$.ajax({
		url:url,
		dataType:'json',
		...
		success:function(result){
			...
			if(sigungu!='' && sigungu!=null){
				...
				comment = list[0].emdNm + ", " + list[1].emdNm;
				last = list[list.length-1].emdNm;
			}else{
				/*광역 종합지수창*/
				for(var i in list){
					var html ='';
				...
				comment = list[0].sggNm + ", " + list[1].sggNm;
				last = list[list.length-1].sggNm;
			}
			
			$('#reportComment').html(" - "+ $('#reportComment').text()+"과 관련된 취약성평가 종합지수가 상대적으로 높은 지역은 <b>"+ comment + "</b> 순으로 도출되었으며, 상대적으로 취약성이 낮은 지역은  <b>" + last + "</b>(이)가 가장 낮은 지역으로 나타남<br/>" );
		}
	});
}
...
function fn_export2Doc(element,filename){
	...
	var preHtml = "<html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'><head><meta charset='utf-8'><title>"+filename+"</title>"
 	+"<link href='http://210.113.102.135:9080/resources/css/bootstrap/bootstrap.css' rel='stylesheet'>"
	+"<link href='http://210.113.102.135:9080/resources/css/developer/style.css' rel='stylesheet'>" 
	+"<link href='http://210.113.102.135:9080/resources/css/developer/style-custom.css' rel='stylesheet'>" 
