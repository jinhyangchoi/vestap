...
<title>VESTAP - 전국단위 취약성 - 누적현황보고</title>
							...
							<div class="row">
								<div class="col">
											...
											<div class="toolTipUl fold-open-sensitive-01">
												<ul class="nav navbar-nav itemList" id="itemList_02_01">
													<c:forEach var="list" items="${list }">
														<c:if test="${list.sectorId eq 'SEC02' }">
										...
										<hr class=" mb-0 m-2">
											<div class="toolTipUl fold-open-sensitive-02">
												<ul class="nav navbar-nav itemList" id="itemList_02_02">
													<c:forEach var="list" items="${list }">
														<c:if test="${list.sectorId eq 'SEC02' }">
															<c:if test="${list.sector2Id ne 'SED01'}">
																<li><a href="#" onclick="fn_itemClick($(this));return false;" data="${list.indiId }" class="toolTip ${list.indiId } ">
																		${list.indiNm } </a></li>
															</c:if>
														</c:if>
									...
									<div class="toolTipUl fold-open-adapt">
										...
										<hr class="m-2">
											<div class="toolTipUl fold-open-adapt-01">
												<ul class="nav navbar-nav itemList" id="itemList_03_01">
													<c:forEach var="list" items="${list }">
														<c:if test="${list.sectorId eq 'SEC03' }">
										...
										<div class="mt-0  pl-2 fold-menu-adapt-03">기타 현황<i class="fas fa-caret-down p-1 pr-3" style="float: right;"></i></div>
										<hr class=" mb-0 m-2">
											<div class="toolTipUl fold-open-adapt-03">
												<ul class="nav navbar-nav itemList" id="itemList_03_03">
													<c:forEach var="list" items="${list }">
														<c:if test="${list.sectorId eq 'SEC03' }">
															<c:if test="${list.sector2Id ne 'SED04' and list.sector2Id ne 'SED03'}">
																<li><a href="#" onclick="fn_itemClick($(this));return false;" data="${list.indiId }" class="toolTip ${list.indiId }">
																		${list.indiNm } </a></li>
															</c:if>
														</c:if>
	...
	<script>
		$(document).ready(function(){
	        ...
	        $(".fold-menu-adapt-02").click(function(){
		     ...
		     // 적응능력 현황 지표 리스트 접펼
		     $(".fold-menu-adapt-03").click(function(){
		    	var submenu = $(".fold-open-adapt-03");
		    	
		    	//submenu가 화면상에 보일때는 위로 보드랍게 접고 아니면 아래로 보드랍게 펼치기
		    	if( submenu.is(":visible")){
		    		submenu.slideUp();
		    		$(".fold-menu-adapt-03 > i").removeClass("fa-caret-up");
		    		$(".fold-menu-adapt-03 > i").addClass("fa-caret-down");
		    		
		    	}else{
		    		submenu.slideDown();
		    		$(".fold-menu-adapt-3 > i").removeClass("fa-caret-down");
		    		$(".fold-menu-adpat-3 > i").addClass("fa-caret-up");
		    	}
		     });
	    });
		$(document).ready(function() {
		...
		function fn_sigunguList(sidoCode) {
			...
			$.ajax({
				url : '/admin/climate/cumulative/sigunguList.do',
				dataType : 'json',
				type : 'get',
				data : dataSet,
		...
		function fn_cumulativeDetail(indiId) {
			...
			$.ajax({
				url: "/admin/climate/cumulative/cumulativeDetail.do",
				data: dataSet,
				dataType:'json',
				type:"get",
				success: function(result){
		...
		function fn_downloadCumulative(name1,name2, sido, sigungu, sidoCode) {	//name : 지표명
			...
			location.href = encodeURI("/admin/climate/cumulative/downloadCumulative.do?" + params);
		}
		...
		function fn_cumulativeTotal() {
			...
			$.ajax({
				url: "/admin/climate/cumulative/cumulativeMain.do",
				data: dataSet,
				dataType:'json',
				type:"get",
				success: function(result){
		...
		function fn_linkSearch() {
			...
			$.ajax({
				url: "/admin/climate/cumulative/keyword.do",
				data: dataSet,
				dataType:'json',
				type:"get",
				success: function(result) {
